...
class TestBaiDu(object):
    ...
    def test_search_2(self, firefox_driver):
        firefox_driver.find_element(*self.locator_kw).send_keys('Python selenium')
        firefox_driver.find_element(*self.locator_su).click()
        time.sleep(2)
        links = firefox_driver.find_elements(*self.locator_result)
        for link in links:
            logger.info(link.text)
